...
function loadHouseholdData() {
  ...
  unsubSnapshot = onSnapshot(doc(db, "households", currentHouseholdId), (docSnap) => {
    if (docSnap.exists()) {
      ...
      setDoc(doc(db, "households", currentHouseholdId), appData);
    }
...
function loadFamilyMembers() {
  // 같은 householdId를 공유하는 유저들의 닉네임을 불러와 리스트 표시
  onSnapshot(query(collection(db, "users"), where("householdId", "==", currentHouseholdId)), (snapshot) => {
    ...
    document.getElementById('family-members-list').textContent = names.join(', ');
  });
}

function saveData() { setDoc(doc(db, "households", currentHouseholdId), appData); }

/* ================= 3. 내 정보, 타이틀 변경, 가족 연동 및 백업 ================= */
...
document.getElementById('btn-change-password').addEventListener('click', () => {
  openModal('비밀번호 변경', `<div class="form-group"><input type="password" id="new-pw" class="form-input" placeholder="새 비밀번호 (6자리 이상)"></div>`, async () => {
    ...
    if(pw.length >= 6) {
      ...
      catch(e) { alert("오류: 로그아웃 후 다시 로그인해야 변경할 수 있습니다."); }
    } else alert("6자리 이상 입력하세요.");
  });
});

document.getElementById('btn-change-ledger-name').addEventListener('click', () => {
...
new Sortable(document.getElementById('tab-home'), { handle: '.drag-handle', animation: 150, onEnd: function () { appData.widgetOrder = Array.from(document.getElementById('tab-home').querySelectorAll('.widget')).map(el => el.id); saveData(); } });
...
function updateHome() {
  ...
  appData.incomes.forEach(i => { totalIncome += Number(i.amount); htmlIncome += `<div class="mini-item"><span class="mini-item-name">${i.name}</span><span class="mini-item-amt blue-text">${formatMoney(i.amount)}</span></div>`; });
  ...
  appData.incomes.forEach(item => {
    ...
    div.innerHTML = `
      <div class="item-info" style="width:100%;">
        ...
        <p style="font-size:12px; color:#888;" onclick="editIncome(${item.id})">매월 ${item.date}일</p>
...
fabBtn.addEventListener('click', () => {
  if (currentIndex === 2) {
    ...
    openModal('새 소득 등록', html, () => {
      appData.incomes.push({ id: Date.now(), name: document.getElementById('i-name').value, date: document.getElementById('i-date').value, taxAmount: getRawNumber(document.getElementById('i-tax').value) || 0, amount: getRawNumber(document.getElementById('i-amt').value) });
      saveData(); window.closeModal();
    });
    attachCommaEvent('i-amt'); attachCommaEvent('i-tax');
  }
  ...
  if (e.target.files.length > 0) {
    ...
    setTimeout(() => {
      ...
      openModal('명세서 등록', html, () => {
        appData.incomes.push({ id: Date.now(), name: document.getElementById('ocr-name').value, date: document.getElementById('ocr-date').value, taxAmount: Number(document.getElementById('ocr-tax').value), amount: Number(document.getElementById('ocr-net').value) });
        saveData(); window.closeModal();
      });
    }, 2000);
  }
...
document.getElementById('btn-logout').addEventListener('click', () => { openModal('로그아웃', `<p style="text-align:center; color:#333; font-weight:500;">정말 로그아웃 하시겠습니까?</p>`, async () => { try { await signOut(auth); window.closeModal(); } catch (error) { alert("로그아웃 실패"); } }); });
document.getElementById('btn-reset-data').addEventListener('click', () => { openModal('데이터 초기화', `<p style="text-align:center; color:#FF4B4B; font-weight:bold;">모든 데이터를 삭제할까요?</p>`, async () => { appData = { expenses: [], incomes: [], categories: defaultCategories, paymentMethods: defaultPaymentMethods, payAccounts: defaultPayAccounts, withdrawAccounts: defaultWithdrawAccounts, widgetOrder: [], ledgerName: "월급 찍고 갑니다" }; await saveData(); window.closeModal(); alert('초기화 완료.'); location.reload(); }); });
